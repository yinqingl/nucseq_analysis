function ydata = tsne_matlab(X, labels, no_dims, initial_dims, perplexity, DISTANCE)
...
% 2. skip internal PCA
% 3. allow weight used in distance calculation, given as precomputed
    ...
    if ~exist('DISTANCE', 'var') || isempty(perplexity)
        DISTANCE = 'euclidean';
    end    
...
%     D = bsxfun(@plus, sum_X, bsxfun(@plus, sum_X', -2 * (X * X')));
    if strcmp(DISTANCE, 'precomputed'),
        ...
        %calculate distance on columns
        D=squareform(pdist(X',DISTANCE));
    end
    
    % Compute joint probabilities
    P = d2p_matlab(D, perplexity, 1e-5);                                           % compute affinities using fixed perplexity
    clear D
...
%     P(1:n + 1:end) = 0;                                 % set diagonal to zero
    ...
    ydata = tsne_p_matlab(P, labels, no_dims);
%     ydata = tsne_p_matlab_slow(P, labels, no_dims);
